...
class ImageAnalyzer:
    ...
    def _initialize_easyocr(self):
        ...
        try:
            ...
            if self.debug:
                print("[ImageAnalyzer] ✅ EasyOCR chargé avec succès (GPU activé)")
    ...
    def analyze_frame(
        ...
        for roi_name in rois_to_analyze:
            roi_result = self._analyze_single_roi_raw(frame, roi_name, preprocessing)
    ...
    def _process_ocr_roi_raw(self, roi_name, roi_image, roi_info, preprocessing):
        ...
        self._log_debug(f"ROI '{roi_name}' uses OCR processing with model: {ocr_model}")

        enhanced_image = self._enhance_roi_image(roi_image, roi_info, preprocessing)
        ...
        return self._extract_raw_text_by_model(enhanced_image, roi_info, roi_name)
    ...
    def _enhance_roi_image(self, roi_image, roi_info, preprocessing):
        ...
        return self.image_converter.enhance_for_ocr(roi_image, roi_info, preprocessing)
    ...
    def _save_debug_frame(self, frame, results):
        ...
        annotated_frame = self.annotate_frame_with_rois(
            frame, list(results.keys()), show_text=True, detection_results=results
        )
    ...
    def visualize_rois(self, image_path, rois_to_show=None):
        ...
        self.annotate_frame_with_rois(image, rois_to_show, show_text=False)
    ...
    def annotate_frame_with_rois(
        self, frame, rois_to_show, show_text=True, detection_results=None
    ):
        ...
        for roi_name in rois_to_show:
            ...
            cv.rectangle(annotated, (bg_x1, bg_y1), (bg_x2, bg_y2), (0, 0, 0), -1)
    ...
    def _extract_roi(self, image, roi_name):
        ...
        if roi_name == "timer" and self.timer_roi:
            ...
            return self._extract_roi_from_config(image, self.character1_roi)
        elif roi_name == "character2" and self.character2_roi:
            return self._extract_roi_from_config(image, self.character2_roi)
        ...
        left_x, top_y, right_x, bottom_y = self._validate_boundaries(
            left_x=boundaries[0],
            top_y=boundaries[1],
            right_x=boundaries[2],
            bottom_y=boundaries[3],
            height=height,
            width=width,
            roi_name=roi_name,
        )
    ...
    def _validate_boundaries(
        self, left_x, top_y, right_x, bottom_y, height, width, roi_name
    ):
    ...
    def _extract_raw_text_trocr(self, enhanced_image):
        ...
        if self.debug:
            print(
                f"[ImageAnalyzer] _extract_raw_text_trocr: processing image shape {enhanced_image.shape}"
            )
        ...
        try:
            if len(enhanced_image.shape) == 3:
                ...
                pil_image = Image.fromarray(
                    cv.cvtColor(enhanced_image, cv.COLOR_BGR2RGB)
                )
            else:
    ...
    def _extract_raw_text_easyocr(self, enhanced_image, roi_info=None):
        ...
        if self.debug:
            print(
                f"[ImageAnalyzer] _extract_raw_text_easyocr: processing image shape {enhanced_image.shape}"
            )
        ...
        try:
            ...
            if self.debug:
                print(
                    f"[ImageAnalyzer] EasyOCR détections multiples: {[(r[1], f'{r[2]:.3f}') for r in results]}"
                )

            # Concaténer toutes les détections triées par position (gauche à droite)
            # Trier par coordonnée X du centre de la bounding box
            sorted_results = sorted(
                results, key=lambda x: (x[0][0][0] + x[0][2][0]) / 2
            )

            # Combiner tous les textes détectés
            detected_texts = [
                result[1].strip() for result in sorted_results if result[1].strip()
            ]
            combined_text = " ".join(detected_texts)

            # Calculer confiance moyenne pour debug
            avg_confidence = sum(result[2] for result in results) / len(results)

            if self.debug:
                individual_texts = [(r[1], f"{r[2]:.3f}") for r in sorted_results]
                print(f"[ImageAnalyzer] EasyOCR textes triés: {individual_texts}")
                print(
                    f"[ImageAnalyzer] EasyOCR texte combiné: '{combined_text}' (confiance moy: {avg_confidence:.3f})"
                )
    ...
    def _analyze_pattern_roi(self, roi_image, roi_info):
        ...
        if not os.path.exists(templates_dir):
            if self.debug:
                print(
                    f"[ImageAnalyzer] ⚠️  Templates directory not found: {templates_dir}"
                )
            return ""
        ...
        for template_file in os.listdir(templates_dir):
            ...
            if template is None:
                if self.debug:
                    print(
                        f"[ImageAnalyzer] ⚠️  Could not load template: {template_path}"
                    )
                continue

            roi_gray = (
                cv.cvtColor(roi_image, cv.COLOR_BGR2GRAY)
            ...
            for method in methods:
                try:
                    ...
                    if self.debug:
                        method_name = {
                            cv.TM_CCOEFF_NORMED: "CCOEFF",
                            cv.TM_CCORR_NORMED: "CCORR",
                            cv.TM_SQDIFF_NORMED: "SQDIFF",
                        }[method]
                        print(
                            f"[ImageAnalyzer]   {template_file} ({method_name}): {confidence:.3f}"
                        )

                except Exception as e:
                    if self.debug:
                        print(
                            f"[ImageAnalyzer] Error matching template {template_file}: {e}"
                        )
                    continue
        ...
        if best_match and best_confidence >= confidence_threshold:
            if self.debug:
                print(
                    f"[ImageAnalyzer] ✅ Best pattern match: '{best_match}' (confidence: {best_confidence:.3f})"
                )
            return best_match
        else:
            if self.debug:
                print(
                    f"[ImageAnalyzer] ❌ No pattern match above threshold {confidence_threshold}"
                )
            return ""
