...
class RoiManager:
    ...
    def get_roi(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        for roi in self._config['rois']:
            if roi['name'] == name:
                return roi.copy()  # Retourner une copie pour éviter les modifications accidentelles
        return None
    ...
    def preview_rois_on_image(self, image: np.ndarray, show_labels: bool = True) -> np.ndarray:
        """
        ...
        for roi in self._config['rois']:
            ...
            if show_labels:
                ...
                cv.rectangle(preview_img, 
                           (text_x - 2, text_y - text_height - 2),
                           (text_x + text_width + 2, text_y + 2),
                           (0, 0, 0), -1)
                
                cv.putText(preview_img, label, (text_x, text_y), font, scale, (255, 255, 255), thickness)
    ...
    @classmethod
    def create_from_scratch(cls, config_file: str, default_rois: Optional[List[Dict]] = None) -> 'RoiManager':
        """
