...
class MatchGrouper:
    ...
    def _group_by_temporal_proximity(self, sorted_sets: List[Dict]) -> List[Dict]:
        """
        Groupe les sets par paires de joueurs (nouvelle logique EVO).

        Un match = tous les sets entre la même paire de joueurs, peu importe :
        - L'ordre des joueurs (KAKERU vs MenaRD = MenaRD vs KAKERU)
        - Les changements de personnages
        - Les gaps temporels (interviews, pauses entre sets d'un match)
        """
        if not sorted_sets:
            return []

        self._log_debug(f"   🎯 NOUVELLE LOGIQUE: Groupage par paires de joueurs")

        # Grouper par paires de joueurs
        player_matches = {}  # {player_pair_key: [sets]}

        for set_data in sorted_sets:
            player_pair = self._extract_player_pair_key(set_data)

            if player_pair:
                if player_pair not in player_matches:
                    player_matches[player_pair] = []
                    self._log_debug(f"   → Nouveau match détecté: {player_pair}")

                player_matches[player_pair].append(set_data)
                self._log_debug(f"   → Set ajouté à {player_pair}: {set_data['start_time'].strftime('%H:%M:%S')}")
            else:
                self._log_debug(f"   ⚠️ Set ignoré (joueurs manquants): {set_data['start_time'].strftime('%H:%M:%S')}")

        # Convertir en matches structurés
        matches = []
        for player_pair, sets in player_matches.items():
            if self._is_valid_match(sets):
                match_data = self._create_match_data(sets)
                matches.append(match_data)
                self._log_debug(f"   ✅ Match {player_pair}: {len(sets)} sets, {self._count_total_rounds(sets)} rounds")
            else:
                self._log_debug(f"   ❌ Match {player_pair} rejeté: {len(sets)} sets (invalide)")

        # Trier les matches par heure de début
        matches.sort(key=lambda m: m['start_time'])

        return matches

    def _extract_player_pair_key(self, set_data: Dict) -> Optional[str]:
        """
        Extrait une clé unique pour une paire de joueurs.

        La clé est normalisée pour que KAKERU vs MenaRD = MenaRD vs KAKERU.
        Format: "player1,player2" avec ordre alphabétique.

        Args:
            set_data: Données du set

        Returns:
            Clé de la paire ou None si joueurs manquants
        """
        player1 = set_data.get('player1', '').strip()
        player2 = set_data.get('player2', '').strip()

        # Ignorer si un joueur manque
        if not player1 or not player2:
            return None

        # Ordre alphabétique pour normalisation
    ...
    def _both_characters_changed(self, existing_sets: List[Dict], new_set: Dict) -> bool:
        """
        Vérifie si ce set représente un nouveau match basé sur les changements de personnages.

        Nouvelle règle EVO: Les joueurs peuvent changer leurs personnages entre sets.
        Un nouveau match commence seulement quand les JOUEURS changent, pas les personnages.
        ...
        Returns:
            True si c'est un nouveau match (joueurs différents), False sinon
        """
        if not existing_sets:
            return False

        # ⚡ NOUVELLE LOGIQUE: Se baser sur les JOUEURS, pas les personnages
        # Les changements de personnages sont autorisés dans un match EVO

        # Si les joueurs sont cohérents, autoriser les changements de personnages
        players_consistent = self._are_players_consistent(existing_sets, new_set)

        if players_consistent:
            # Même joueurs = même match, peu importe les personnages
            self._log_debug(f"   ✅ Personnages changés mais joueurs cohérents = MÊME MATCH")
            return False

        # Si les joueurs ne sont pas cohérents, c'est un nouveau match
        self._log_debug(f"   🚫 Joueurs incohérents = NOUVEAU MATCH")
        return True
